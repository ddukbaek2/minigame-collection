...
try {
	...
	if (!fileSystem.existsSync(androidDirectory)) {
		...
		if (!fileSystem.existsSync(wwwIndexPath)) {
			console.error(`[buildapk] ${wwwIndexPath} 가 없습니다.`);
			console.error(`[buildapk] 루트에서 'npm run build' 를 먼저 실행해 build/web 에 index.html 포함된 산출물을 준비하세요.`);
			exitCode = 1;
			throw new Error("[buildapk] www/index.html 미존재.");
		}

		console.log("[buildapk] android/ 디렉토리가 없어 'npx cap add android' 를 실행합니다.");
		...
		if (addResult.status !== 0) {
			...
			if (fileSystem.existsSync(androidDirectory)) {
				...
				console.warn("[buildapk] cap add android 부분 실패로 android/ 를 제거했습니다.");
			}
			exitCode = addResult.status === null ? 1 : addResult.status;
			throw new Error("[buildapk] cap add android 실패. Android SDK / JDK 설치 상태를 확인하세요.");
		}
	}
	...
	if (!fileSystem.existsSync(localPropertiesPath)) {
		...
		if (sdkDir) {
			...
			console.log(`[buildapk] local.properties 생성: sdk.dir=${normalizedSdkDir}`);
		}
		else {
			console.warn("[buildapk] Android SDK 경로를 자동 감지하지 못했습니다.");
	...
	if (fileSystem.existsSync(gradlePropertiesPath)) {
		...
		if (!gradleProperties.includes("org.gradle.java.home")) {
			if (!fileSystem.existsSync(jbrPath)) {
				console.warn(`[buildapk] Android Studio 번들 JBR 경로가 없습니다: ${jbrPath}`);
				console.warn("[buildapk] Android Studio 를 설치했거나, gradle.properties 에 올바른 'org.gradle.java.home' 경로를 수동으로 지정하세요.");
			}
			else {
				...
				console.log(`[buildapk] gradle.properties 에 org.gradle.java.home=${jbrPath} 를 추가했습니다.`);
			}
		}
	}
